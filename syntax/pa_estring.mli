...
(** In order for string to be converted by [pa_estring] we need to add
    to strings which specifier they use. We call tagged a string with
    this information added.

    Basically all strings that appears in the source code will be
    tagged, and the strings not tagged are the ones generated by
    syntax extensions. *)

val tag : char -> string -> string
  (** [tag spec string] tag [str] with the specifier [spec] *)

val untag : string -> (char * string) option
  (** [untag str] if [str] is a tagged string, then @return its
      specifier and the string without the tag, otherwise @return
      [None] *)
